...
int jolastu(JOKALARIA* jokalaria)
{
    ...
    int egun = 1;//egun bakoitzean dena berrabiaraziko da(galderak) ||| 1. eta 2. egunak normal eta 3. azterketa
    ...
    while (irten == 3)
    {
        ...
        if (ebentu == SAGU_BOTOIA_EZKERRA) {
            ...
            gelaID = 1;//------------------------------------------------------------------------------------------------------------------------------------CAMBIAR a 1
            galderaID = 1;//---------------------------------------------------------------------------------------------------------------------------------CAMBIAR a 1

            error = galderakEtaAukerakLotuFitxategiarenBitartez(galdera);

            if (error == 1) {//---FITXATEGIAK GAIZKI IRAKURTZEN BADU JOKOA SEGI AHAL IZATEKO---copia de seguridad de las galderas
        ...
        {
            //funcion cama (guardar)
            //si no ha llegado al max no sube
            if (jokalaria->gradua.exp.max == jokalaria->gradua.exp.xp)
            {
                //jokalaria->gradua.exp.max += 10;
...
int galderakEtaAukerakLotuFitxategiarenBitartez(GALDERA galdera[GELAIDMAX][GALDERAIDMAX]) {
    ...
    char galderak[60][128] = {
        ...
        GALDERA_NULL,PROGRAM_II_GALDERA_1,PROGRAM_II_GALDERA_2,PROGRAM_II_GALDERA_3,PROGRAM_II_GALDERA_4,PROGRAM_II_GALDERA_5,
            ...
            case 8://---Mate D---
                ...
                case 5:
                    ...
                    if ((pos.x >= 34 && pos.x <= 628) && (pos.y >= 493 && pos.y <= 569)) { //-------------POS A------------------
                        ...
                        irudiaMugitu(irudiaKargatu(".\\img\\Galderak_Erantzunak\\MateD\\MD5C.bmp"), 30, 614);
                        irudiakMarraztu();
                        opzioakAgertu(galdera, gelaID, galderaID, 3);
                    }

                    else if ((pos.x >= 659 && pos.x <= 1255) && (pos.y >= 616 && pos.y <= 694)) { //-------------POS D-----------------
                        irudiaMugitu(irudiaKargatu(".\\img\\Galderak_Erantzunak\\MateD\\MD5D.bmp"), 656, 616);
                        irudiakMarraztu();
                        opzioakAgertu(galdera, gelaID, galderaID, 4);
                    }
